...
board_t *copy_board(board_t *board)
{
    ...
    // We can do that because we never modify the graph
    new_board->graph = copy_graph(board->graph);
    new_board->num_accessible_vertices = board->num_accessible_vertices;
    new_board->num_queens = board->num_queens;
    for (unsigned int i = 0; i < NUM_PLAYERS; i++)
    {
        new_board->queens[i] = malloc(sizeof(unsigned int) * board->num_queens);
        memcpy(new_board->queens[i], board->queens[i], (sizeof(unsigned int)*board->num_queens));
    }
    new_board->arrows = malloc(sizeof(unsigned int) * board->graph->num_vertices);
    memcpy(new_board->arrows, board->arrows, sizeof(unsigned int) * board->graph->num_vertices);
    new_board->queens_on_board = malloc(sizeof(unsigned int) * board->graph->num_vertices);
    memcpy(new_board->queens_on_board, board->queens_on_board, sizeof(unsigned int) * board->graph->num_vertices);
